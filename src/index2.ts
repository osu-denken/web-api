import { base642txt,
  createJsonResponse2, createJsonResponseRaw,
		...
		try {
			...
			if (pathname === "/invite/create") {
				if (request.method !== "POST") {
					return createJsonResponse2(405, "Method Not Allowed", { error: "Only POST method is allowed" });
				}

				let idToken = request.headers.get("Authorization");
				if (!idToken) {
					return createJsonResponse2(401, "Unauthorized", { error: "Authorization header is required" });
				}
			...
			if (pathname === "/invite/validate") {
				...
				if (!code) {
					return createJsonResponse2(400, "Bad Request", { error: "code is required" });
				}
			...
			if (pathname === "/user/exists") {
				...
				if (!email) {
					return createJsonResponse2(400, "Bad Request", { error: "email is required" });
				}

				if (!email.includes("@")) email = email + `@ge.osaka-sandai.ac.jp`;

				if (!email.match(/@(.+?)\.osaka-sandai\.ac\.jp$/)) {
					return createJsonResponse2(400, "Bad Request", { error: "Email must be from osaka-sandai.ac.jp domain" });
				}
			...
			if (pathname === "/user/register") {
				...
				if (passphrase !== env.REGISTER_PASSPHRASE) {
					...
					if (!localId) {
						return createJsonResponse2(403, "Forbidden", { error: "Invalid passphrase or invite code" });
					}
				}
				
				if (!email || !password) {
					return createJsonResponse2(400, "Bad Request", { error: "email and password are required" });
				}
			...
			if (pathname === "/portal") {
				...
				if (!verifyData) return createJsonResponse2(401, "Unauthorized", { error: "Invalid idToken" });
			...
			if (pathname === "/v1/blog/get") {
				...
				if (!page) return createJsonResponse2(400, "Bad Request", { error: "page parameter is required" });
				...
				if (!data.content) {
					if (data.status === 404) {
						return createJsonResponse2(404, "Not Found", { error: "Post not found" });
					}
					
					return createJsonResponse2(404, "Not Found", { error: "Post not found", data });
				}
			...
			if (pathname === "/v2/blog/get") {
				let page = url.searchParams.get("page");
				if (!page) {
					return createJsonResponse2(400, "Bad Request", { error: "page parameter is required" });
				}
			...
			if (pathname === "/test") {
				...
				if (token.replace("Bearer ", "") !== env.AUTH_TOKEN) {
					return createJsonResponse2(403, "Forbidden", { error: "Invalid authorization token" });
				}
			...
			if (pathname === "/auth") {
				...
				if (data.idToken) {
					...
					return createJsonResponse2(403, "Forbidden", { error: "Invalid email or password" });
				}
			}
			...
			if (pathname === "/v1/blog/update") {
				...
				if (!page || !content) {
					return createJsonResponse2(400, "Bad Request", { error: "page and content headers are required" });
				}
			...
			if (pathname === "/v2/blog/update") {
				...
				if (!page || !_content || !_meta) {
					return createJsonResponse2(400, "Bad Request", { error: "page, meta and content headers are required" });
				}
				...
				try {
					...
					return createJsonResponse2(400, "Bad Request", { error: "meta header is not valid JSON" });
				}
			...
			if (pathname === "/discord/invite") {
				...
				if ((data.error || !data.localId)) {
					return createJsonResponse2(401, "Unauthorized", { error: "Invalid idToken", data: data });
				}
			...
			if (pathname === "/github/invite") {
				if (request.method !== "POST") {
					return createJsonResponse2(405, "Method Not Allowed", {
				...
				if (!res.ok) {
					return createJsonResponse2(res.status, res.statusText, {
			...
			return createJsonResponse2(500, "Internal Server Error", { error: e.toString() });
		}

		return new HttpError(404, "NOT_FOUND", "Endpoint not found").toResponse();
	}
